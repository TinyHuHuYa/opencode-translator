...
`/t` 通过 `源语言:目标语言` 语法（第 5.1 节）提供 `from`，`translate` 工具通过 `from` 参数提供；两者都省略或为空时走第一种情况。
...
1. 移除语言参数之前的空白。
2. 如果语言参数以引号开头，则使用标准反斜杠转义规则读取到配对引号。
3. 否则，将第一个由空白分隔的 token 作为语言参数。
4. 语言参数按第一个冒号（`:` 或全角 `：`）切分为 `源语言` 与 `目标语言`，两侧各自去除首尾空白；无冒号时整个参数即为目标语言，源语言留空。
5. 源语言为空时等同未提供，走自动识别；渲染规则见第 4 节。
6. 仅移除语言参数与文本之间的分隔空白。
7. 保留其余全部文本，包括换行、标记、代码和末尾空白。
8. 在调用模型之前拒绝缺少目标语言或源文本为空的请求。
